...
import { setupSessionRoomMain, type SessionRoomOptions } from './main/index.js';
...
export function createSessionRoomMain(options?: SessionRoomOptions) {
  return createPlugin({
    name: () => 'session-room',
    description: () => 'Collaborative listening sessions over Supabase Realtime',
    backend: { start: (ctx) => setupSessionRoomMain(ctx, options) },
  });
}
