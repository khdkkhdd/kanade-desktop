...
- **다수 가능**: 매시업/메들리의 경우 원곡이 여러 개 — 무한스크롤 대비
...
- **무한스크롤**: 고정 높이 컨테이너 + 내부 스크롤, 스크롤 끝 도달 시 다음 페이지 fetch
...
- **칩 동작**:
  ...
  - 다른 칩 클릭 → 전환
  - 현재 아티스트가 기본 선택 상태
- **영상 목록**: 무한스크롤 (고정 높이 + 내부 스크롤)
- **클릭**: YouTube SPA 내비게이션

### 리사이즈 대응

Electron 윈도우 리사이즈 시 패널이 깨지지 않도록 대응한다. 최소 너비(325px) 기준으로 썸네일 + 텍스트가 한 행에 유지되도록 한다.
...
- **무한스크롤 추가 호출**: renderer가 IPC로 offset/limit 요청 → Main에서 fetch → 결과 반환

### DOM Injection

- **위치**: 영상 설명란(`#below` 컨테이너) 아래, 댓글 섹션 위
